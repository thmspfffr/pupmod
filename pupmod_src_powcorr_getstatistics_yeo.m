function [outp] = pupmod_src_powcorr_getstatistics_yeo(para)
...
%        frequency and efload /home/tpfeffer/pconn/proc/src/pconn_sa_s4_m1_b1_v9.mat
%   (2c)
%
% ------------------
...
if ~isfield(para,'emp')
  ...
  emp = para.emp;
  para.fcsize = size(emp{1}.n_p_atx_pervoxel,1);
end
...
% --------------------------
  ...
  for iperm = 1 : allperms
    ...
    load(sprintf('~/pupmod/proc/conn/pupmod_src_powcorr_permtest_yeo_iperm%d_nperm%d_v%d.mat',iperm,para.nperm,para.ver),'par')
    
    % --------------
    % ATOMOXETINE: global effects
    % --------------
    % rest
    perm_n_p_atx((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)   = par(iyeo).tperm_res1_p;
    perm_n_n_atx((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)   = par(iyeo).tperm_res1_n;
    % task
    perm_n_p_atx((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,2)   = par(iyeo).tperm_cnt1_p;
    perm_n_n_atx((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,2)   = par(iyeo).tperm_cnt1_n;
    % irrespective of context
%     perm_n_all_atx((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:,1) = par(iyeo).tperm_atx_during_rest;
%     perm_n_all_atx((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:,2) = par(iyeo).tperm_atx_during_task;
    % context-dependence
    context_allconn_atx_p((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)=par(iyeo).tperm_res1_p-par(iyeo).tperm_cnt1_p;
    context_allconn_atx_n((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)=par(iyeo).tperm_res1_n-par(iyeo).tperm_cnt1_n;
    
    % --------------
    % DONEPEZIL
    % --------------
    % rest
    perm_n_p_dpz((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)   = par(iyeo).tperm_res2_p;
    perm_n_p_dpz((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,2)   = par(iyeo).tperm_cnt2_p;
    % task
    perm_n_n_dpz((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)   = par(iyeo).tperm_res2_n;
    perm_n_n_dpz((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,2)   = par(iyeo).tperm_cnt2_n;
    % irrespective of context
%     perm_n_all_dpz((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:,1) = par(iyeo).tperm_dpz_during_rest;
%     perm_n_all_dpz((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:,2) = par(iyeo).tperm_dpz_during_task;
    % context-dependence
    context_allconn_dpz_p((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)=par(iyeo).tperm_res2_p-par(iyeo).tperm_cnt2_p;
    context_allconn_dpz_n((iperm-1)*par(1).subs+1:(iperm)*par(1).subs,:,1)=par(iyeo).tperm_res2_n-par(iyeo).tperm_cnt2_n;
    
    % TASK VS REST
    % --------------
    % global effects
%     perm_taskvsrest_n_p((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:) = par(iyeo).tperm_taskvsrest_p;
%     perm_taskvsrest_n_n((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:) = par(iyeo).tperm_taskvsrest_n;
    
    % double dissociation
    % --------------
%     perm_doubledissociation((iperm-1)*par(iyeo).subs+1:(iperm)*par(iyeo).subs,:) = par(iyeo).tperm_doubledissociation;
  end
  %%
...
%     Rmax_tvr_p = max(R_tvr_p,[],2);
    ...
    Dmax_context1_p_corr  = zeros(para.nperm,13);
    Dmax_context1_n_corr  = zeros(para.nperm,13);
    ...
    Dmax_context2_p_corr  = zeros(para.nperm,13);
    Dmax_context2_n_corr  = zeros(para.nperm,13);
...
%         Dmax_tvr_p_corr(irank,ifreq) = perm_taskvsrest_n_n( R_tvr_p(:,ifreq) == Rmax_tvr_p(irank), ifreq);
      ...
      outp(iyeo).p_res1_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_atx(ifreq,1))>abs(Dmax_res1_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_res1_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_atx(ifreq,1))>abs(Dmax_res1_n_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_cnt1_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_atx(ifreq,2))>abs(Dmax_cnt1_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_cnt1_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_atx(ifreq,2))>abs(Dmax_cnt1_n_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_context1_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_context_atx(ifreq))>abs(Dmax_context1_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_context1_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_context_atx(ifreq))>abs(Dmax_context1_n_corr(:,ifreq)))/para.nperm;
      
      outp(iyeo).p_res2_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_dpz(ifreq,1))>abs(Dmax_res2_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_res2_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_dpz(ifreq,1))>abs(Dmax_res2_n_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_cnt2_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_dpz(ifreq,2))>abs(Dmax_cnt2_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_cnt2_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_dpz(ifreq,2))>abs(Dmax_cnt2_n_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_context2_p(ifreq) = 1-sum(abs(emp{iyeo}.n_p_context_dpz(ifreq))>abs(Dmax_context2_p_corr(:,ifreq)))/para.nperm;
      outp(iyeo).p_context2_n(ifreq) = 1-sum(abs(emp{iyeo}.n_n_context_dpz(ifreq))>abs(Dmax_context2_n_corr(:,ifreq)))/para.nperm;
      
%       outp(iyeo).p_tvr_p(ifreq) = 1-sum(emp(iyeo).taskvsrest_p(ifreq)>Dmax_tvr_p_corr(:,ifreq))/para.nperm;
%       outp(iyeo).p_tvr_n(ifreq) = 1-sum(emp(iyeo).taskvsrest_n(ifreq)>Dmax_tvr_n_corr(:,ifreq))/para.nperm;
    end
...
%       outp(iyeo).pval_taskvsrest_n_corr(ifreq) = 1-sum(abs(emp(iyeo).taskvsrest_n(ifreq))>idx_R_taskvsrest_n(:,ifreq))/nperm;
    end
  end
