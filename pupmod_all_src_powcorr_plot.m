...
for ifoi = 1:13
    ...
    h=ttest(s_fc(:,:,:,3,1),s_fc(:,:,:,1,1),'dim',3,'alpha',ALPHA);
    dpz(ifoi,1) = nansum(nansum((h)))./(size(s_fc,1)*size(s_fc,1)-size(s_fc,1));
    
    h=ttest(s_fc(:,:,:,3,2),s_fc(:,:,:,1,2),'dim',3,'alpha',ALPHA);
...
for ifreq = 1 : 13
  for irank = 1 : length(idx_R_cnt1_p)
    ...
    %
    % TASK VS REST ------------------------------------------------------
    [t_tvsr,~,~,s] = ttest(taskvsrest_perm(:,:,:,2,ifoi),taskvsrest_perm(:,:,:,1,ifoi),'dim',3,'alpha',ALPHA);
    t_tvsr = t_tvsr.*sign(s.tstat); clear s
