...
for ifoi = 1 : 1
  for m = 1 : 3
    for isubj = SUBJLIST
%       
      if ~exist(sprintf([outdir 'pupmod_task_prepdata_aval_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
        system(['touch ' outdir sprintf('pupmod_task_prepdata_aval_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
      else
        continue
      end
      ...
      for iblock = 1:2
        ...
        load(sprintf('~/pconn_cnt/proc/preproc/pconn_cnt_postpostproc_s%d_m%d_b%d_v%d.mat',isubj,m,iblock,1));
        ...
        load(sprintf(['~/pconn/proc/src/' 'pconn_cnt_src_common_filter_s%d_f%d_v%d.mat'],isubj,6,1));
        ...
        dat = dat*A1;
%                     
        save(sprintf([outdir 'pupmod_task_prepdata_aval_s%d_b%d_m%d_f%d_v%d.mat'],isubj,iblock,m,ifoi,v),'dat','-v7.3');
%         
        clear par A1
