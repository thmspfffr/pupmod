...
% %         
      if ~exist(sprintf([outdir 'pupmod_task_src_psi_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
        system(['touch ' outdir sprintf('pupmod_task_src_psi_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
      else
        continue
      end
%       
      ...
      for iblock = 1:2
        ...
        try 
          ...
          if ~exist(sprintf('/home/tpfeffer/pconn_cnt/proc/preproc/pconn_cnt_postpostproc_s%d_m%d_b%d_v%d.mat',isubj,m,iblock,1))
            psi = nan(90,90);
            save(sprintf([outdir 'pupmod_task_src_psi_s%d_m%d_b%d_f%d_v%d.mat'],isubj,m,iblock,ifoi,v),'psi');
            continue
          else
            error('Data corrupt?')
          end
  ...
  for m = 1 : 3
    for ifoi = 1 : length(foi_range)
      ...
      for iblock = 1 : 2
       ...
       psi_res_all(:,:,isubj,m,ifoi,iblock) = psi;
       
       load(sprintf([outdir 'pupmod_task_src_psi_s%d_m%d_b%d_f%d_v%d.mat'],isubj,im,iblock,ifoi,v));
       
       psi_cnt_all(:,:,isubj,m,ifoi,iblock) = psi;
...
psi_res_all = squeeze(psi_res_all(:,:,SUBJLIST,:,:,:));
psi_cnt_all = squeeze(psi_cnt_all(:,:,SUBJLIST,:,:,:));
...
[h,~,~,s]= ttest(zeros(90,90,28),psi_res_all(:,:,:,1),'dim',3);

send = nanmean(psi_res_all(:,:,:,1),3)>0;
reic = nanmean(psi_res_all(:,:,:,1),3)<0;
