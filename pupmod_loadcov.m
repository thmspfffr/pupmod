...
elseif v == 12 || v == 19 || v ==23
  siz = 400;
  fc = zeros(siz,siz,length(SUBJLIST),3,2,21,2,'single');
  freqs = [1:21];
elseif v == 20  
  siz = 46;
...
%       for ifoi = 1 : 13
          ...
          load(sprintf('~/pupmod/proc/conn/pupmod_src_covariance_s%d_m%d_v%d.mat',SUBJLIST(isubj),im,v))
          fc(:,:,isubj,m,1,:,:) = single(permute(covariance(:,:,:,freqs),[1 2 4 3])); clear covariance

          load(sprintf('~/pupmod/proc/conn/pupmod_task_src_covariance_s%d_m%d_v%d.mat',SUBJLIST(isubj),im,v))
          fc(:,:,isubj,m,2,:,:) = single(permute(covariance(:,:,:,freqs),[1 2 4 3])); clear covariance
        %       end
    end
  end
end
