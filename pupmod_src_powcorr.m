...
% para.wavelet    = 'ft';
% --------------------------------------------------------
% VERSION 5 - new wavelets & lambda = 5% (instead of 1%) - timevariant
% --------------------------------------------------------
% v               = 8;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
...
% para.scnd_filt  = 0;
% --------------------------------------------------------
% VERSION 14 - same as v10, just with coh
% --------------------------------------------------------
% v               = 14;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
...
% para.scnd_filt  = 0;
% --------------------------------------------------------
% VERSION 17 - same as v14, just with zero-padding
% --------------------------------------------------------
% v               = 17;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
...
% allpara.tau     = inf;
% --------------------------------------------------------
% VERSION 101 - like 10 but envelope extracted AFTER spatial filtering
% --------------------------------------------------------
% v               = 101;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
    ...
    for m = 1 : 3
      ...
      for iblock = 1 : 2
        ...
        try
          ...
          s2(:,:,iblock) = powcorr; clear powcorr
        catch me
          s2(:,:,iblock) = nan(90,90);
        end
...
%         
        load(sprintf(['~/pupmod/proc/' 'pupmod_src_powcorr_s%d_m%d_b%d_f%d_v%d.mat'],isubj,im,iblock,ifoi,v));
        
        s1(:,:,iblock) = powcorr; clear powcorr

%         s(:,:,isubj,m,ifoi,iblock) = mean(powcorr,3);
%         fc_mean(isubj,m,ifoi,iblock) = nanmean(powcorr(:));
      ...
      s1_all(:,:,ifoi,isubj,m) = s1;
     	s2_all(:,:,ifoi,isubj,m) = s2;

        p1(ifoi,m,isubj) = nanmean(s1(:));
        p2(ifoi,m,isubj) = nanmean(s2(:));
...
%         clear s1 s2
   ...
   s1_all = s1_all(:,:,:,SUBJLIST,:);
   s2_all = s2_all(:,:,:,SUBJLIST,:);

  p1 = p1(:,:,SUBJLIST)
    p2 = p2(:,:,SUBJLIST);
    
    figure; set(gcf,'color','white');hold on;
    plot(1:13,squeeze(p1(:,2,:)-p1(:,1,:)),'r-')
    plot(squeeze(p2(:,3,:)-p2(:,1,:)),'b--')

%     s1 = squeeze(nanmean(nanmean(s1(:,:,SUBJLIST,:,:),5),3));
%     s2 = squeeze(nanmean(nanmean(s2(:,:,SUBJLIST,:,:),5),3));
% 
%     p1(ifoi,:,isubj) = [nanmean(nanmean(s1(:,:,1))) nanmean(nanmean(s1(:,:,2))) nanmean(nanmean(s1(:,:,3)))];
%     p2(ifoi,:,isubj) = [nanmean(nanmean(s2(:,:,1))) nanmean(nanmean(s2(:,:,2))) nanmean(nanmean(s2(:,:,3)))];
%     p1(ifoi,:) = [nanmean(nanmean(s1(:,:,1))) nanmean(nanmean(s1(:,:,2))) nanmean(nanmean(s1(:,:,3)))];
%     p1(ifoi,:) = [nanmean(nanmean(s1(:,:,1))) nanmean(nanmean(s1(:,:,2))) nanmean(nanmean(s1(:,:,3)))];

%     figure; set(gcf,'color','white'); 
%     subplot(1,2,1)
%     imagesc(s1(:,:,2)-s1(:,:,1),[-0.02 0.02]); axis square; tp_editplots; 
%     subplot(1,2,2)
%     imagesc(s2(:,:,2)-s2(:,:,1),[-0.02 0.02]); axis square; tp_editplots; colormap(cmap)
%    
%     print(gcf,'-djpeg100',sprintf('~/pupmod/plots/pupmod_powcorr_fc_c1_f%d_v%d.jpg',ifoi,v))
%     
%     figure; set(gcf,'color','white'); 
%     subplot(1,2,1)
%     imagesc(s1(:,:,3)-s1(:,:,1),[-0.02 0.02]); axis square; tp_editplots; 
%     subplot(1,2,2)
%     imagesc(s2(:,:,3)-s2(:,:,1),[-0.02 0.02]); axis square; tp_editplots; colormap(cmap)
%    
%     print(gcf,'-djpeg100',sprintf('~/pupmod/plots/pupmod_powcorr_fc_c2_f%d_v%d.jpg',ifoi,v))
...
% EXAMPLE SUBJECT
% figure; set(gcf,'color','white'); imagesc(powcorr); axis square; tp_editplots; 
...
for isubj = 1 : 26
  ...
  s1_mean     = nanmean(nanmean(s1_all(:,:,ifoi,isubj,1)));
  
  s1_hi_idx  = s1_all(:,:,ifoi,isubj,1)>s1_mean;
  s1_lo_idx  = s1_all(:,:,ifoi,isubj,1)<s1_mean;

  s2_mean     = nanmean(nanmean(s2_all(:,:,ifoi,isubj,1)));

  s2_hi_idx  = s2_all(:,:,ifoi,isubj,1)>s2_mean;
  s2_lo_idx  = s2_all(:,:,ifoi,isubj,1)<s2_mean;
  
  for i = 1 : 90
    
    tmp11_hi(i) = nanmean(s1_all(i,logical(s1_hi_idx(i,:)),ifoi,isubj,2)-s1_all(i,logical(s1_hi_idx(i,:)),ifoi,isubj,1));
    tmp11_lo(i) = nanmean(s1_all(i,logical(s1_lo_idx(i,:)),ifoi,isubj,2)-s1_all(i,logical(s1_lo_idx(i,:)),ifoi,isubj,1));
    
    tmp12_hi(i) = nanmean(s1_all(i,logical(s1_hi_idx(i,:)),ifoi,isubj,3)-s1_all(i,logical(s1_hi_idx(i,:)),ifoi,isubj,1));
    tmp12_lo(i) = nanmean(s1_all(i,logical(s1_lo_idx(i,:)),ifoi,isubj,3)-s1_all(i,logical(s1_lo_idx(i,:)),ifoi,isubj,1));

    tmp21_hi(i) = nanmean(s2_all(i,logical(s2_hi_idx(i,:)),ifoi,isubj,2)-s2_all(i,logical(s2_hi_idx(i,:)),ifoi,isubj,1));
    tmp21_lo(i) = nanmean(s2_all(i,logical(s2_lo_idx(i,:)),ifoi,isubj,2)-s2_all(i,logical(s2_lo_idx(i,:)),ifoi,isubj,1));

    tmp22_hi(i) = nanmean(s2_all(i,logical(s2_hi_idx(i,:)),ifoi,isubj,3)-s2_all(i,logical(s2_hi_idx(i,:)),ifoi,isubj,1));
    tmp22_lo(i) = nanmean(s2_all(i,logical(s2_lo_idx(i,:)),ifoi,isubj,3)-s2_all(i,logical(s2_lo_idx(i,:)),ifoi,isubj,1));

  end
  
  d11_hi(isubj) = nanmean(tmp11_hi);
  d11_lo(isubj) = nanmean(tmp11_lo);

  d12_hi(isubj) = nanmean(tmp12_hi);
  d12_lo(isubj) = nanmean(tmp12_lo);
  
  d21_hi(isubj) = nanmean(tmp21_hi);
  d21_lo(isubj) = nanmean(tmp21_lo);
  
  d22_hi(isubj) = nanmean(tmp22_hi);
  d22_lo(isubj) = nanmean(tmp22_lo);
