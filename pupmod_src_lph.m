...
for isubj = SUBJLIST
  for m = 1 : 3
    for ifoi = 1:length(foi_range)
      ...
      if ~exist(sprintf([outdir 'pupmod_src_lph_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
        system(['touch ' outdir sprintf('pupmod_src_lph_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
      else
        continue
      end
      ...
      for iblock = 1:2
        ...
        pars.epleng    = size(dat,2);
        pars.aal       = 0;
        
        % COMPUTE POWER CORRELATIONS
        % dat should be n x nchans
         fprintf('Computing LPC...\n');

        tmp = data2laggedphasecoherence(dat',pars.segleng,pars.segshift,pars.epleng,pars.foi,pars.fsample,filt,filt);
        fprintf('Done.\n');
          % Convert to AAL
        pars = [];
        pars.grid = allpara.grid;
        ...
        save(sprintf([outdir 'pupmod_src_lph_s%d_m%d_b%d_f%d_v%d.mat'],isubj,m,iblock,ifoi,v),'powcorr');
