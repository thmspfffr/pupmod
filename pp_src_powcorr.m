...
for isubj = SUBJLIST
  for m = 1:3
    for ifoi = 1:length(foi_range)
      ...
      if ~exist(sprintf([outdir 'pp_src_powcorr_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
        system(['touch ' outdir sprintf('pp_src_powcorr_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
      else
        continue
      end
