...
for isubj = SUBJLIST
  for m = 1:3
% %     %
    if ~exist(sprintf([outdir 'pupmod_task_src_coherence_s%d_m%d_v%d_processing.txt'],isubj,m,v))
      system(['touch ' outdir sprintf('pupmod_task_src_coherence_s%d_m%d_v%d_processing.txt',isubj,m,v)]);
    else
      continue
    end
    %    
    for iblock = 1:2
      ...
      for ifoi = 1:length(foi_range)
       ...
       cs=sa.sa.filt'*cs*sa.sa.filt;
       coh=(abs(cs./sqrt(diag(cs)*diag(cs)')));
...
%        covariance = single(covariance); 
    ...
    save(sprintf([outdir 'pupmod_task_src_coherence_s%d_m%d_v%d.mat'],isubj,m,v),'coh');
