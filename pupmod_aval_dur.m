    ...
    for m = 1 : 3
      for itau = 1 : length(tau)
        for iT = 1 : length(T)
          ...
          if ~exist(sprintf(['~/pupmod/proc/pupmod_aval_dur_s%d_b%d_m%d_tau%d_T%d_v%d_processing.txt'],isubj,iblock,m,itau,iT,v))
            system(['touch ' '~/pupmod/proc/' sprintf('pupmod_aval_dur_s%d_b%d_m%d_tau%d_T%d_v%d_processing.txt',isubj,iblock,m,itau,iT,v)]);
          else
            continue
          end
    ...
    for m = 1 : 3
      ...
      for itau = 1 : length(tau)
        for iT = 1 : length(T)
          ...
          s=pconn_regress(log10(par.b(idx)),log10(tmp(idx))');
...
slp = squeeze(nanmean(slp(4,iblock,m,:,:),2));
%%
