...
v = 1;
fsample  = 400;
para.bif = -0.2:0.01:0.1;
para.G   = 1:1:25;
para.sig = 0.01:0.01:0.05;
para.freq = random('norm',10,.2,90,1);
% --------------------------------------------------------
  ...
  for isig = 1 : length(para.sig)
    for ibi = 1 : length(para.bif)
      for iG = 1 : length(para.G)
        ...
        save(sprintf([outdir '/pupmod_fc_sim_f%d_s%d_b%d_g%d_v%d.mat'],ifoi,isig,ibi,iG,v),'fcd','fc_sim','kura');
...
for iep = 1 : nep
  for jep = 1 : nep
    ...
    par.fcd(iep,jep) = corr(nonzeros(triu(fc_sim(:,:,iep),1)),nonzeros(triu(fc_sim(:,:,jep),1)));
