...
for isubj = SUBJLIST
  for m = 1:3
    ...
    % ------------
    if ~exist(sprintf([outdir 'pupmod_task_sens_powcorr_s%d_m%d_v%d_processing.txt'],isubj,m,v))
      system(['touch ' outdir sprintf('pupmod_task_sens_src_powcorr_s%d_m%d_v%d_processing.txt',isubj,m,v)]);
    else
      continue
    end
%     
    ...
    save(sprintf([outdir 'pupmod_task_sens_powcorr_s%d_m%d_v%d.mat'],isubj,m,v),'powcorr');
    clear powcorr        
