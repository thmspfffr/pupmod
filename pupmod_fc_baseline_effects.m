...
% --------------------------
% Rest
tmp = squeeze(nanmean(nanmean(fc(:,:,:,[1 2],[1 2],6),2),1));
...
% Rest
tmp = squeeze(nanmean(nanmean(fc(:,:,:,[1 3],[1 2],7),2),1));
...
baseline = squeeze(fc(:,:,isubj,[1],2,6,[1 2]));

for isubj = 1 : 28
  
  tmp = squeeze(fc(:,:,isubj,[1 2],2,6,[1 2]));
...
clear tmp
for isubj = 1 : 28
  tmp_fc = nanmean(fc(:,:,isubj,2,2,6,:),7)-nanmean(fc(:,:,isubj,1,2,6,:),7);
...
% ATX
clear tmp
tmp_fc1 = squeeze(nanmean(nanmean(fc(:,:,:,2,2,6,1)-fc(:,:,:,1,2,6,1),1),2));
tmp_fc2 = squeeze(nanmean(nanmean(fc(:,:,:,2,2,6,2)-fc(:,:,:,1,2,6,2),1),2));
...
% ATX
clear tmp
tmp_fc1 = squeeze(nanmean(nanmean(fc(:,:,:,3,1,7,1)-fc(:,:,:,1,1,7,1),1),2));
tmp_fc2 = squeeze(nanmean(nanmean(fc(:,:,:,3,1,7,2)-fc(:,:,:,1,1,7,2),1),2));
