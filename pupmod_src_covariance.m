...
for isubj = SUBJLIST
  for m = 1:3
%     %
    if ~exist(sprintf([outdir 'pupmod_src_covariance_s%d_m%d_v%d_processing.txt'],isubj,m,v))
      system(['touch ' outdir sprintf('pupmod_src_covariance_s%d_m%d_v%d_processing.txt',isubj,m,v)]);
    else
      continue
    end
%     %    
      ...
      try
        ...
        if ~exist(sprintf('~/pp/proc/pp_sens_cleandat_s%d_m%d_b%d_v%d.mat',isubj,m,iblock,1))
          if strcmp(allpara.grid,'cortex_lowres')
            covariance(:,:,iblock,:) = nan(400,400,1,length(foi_range));
          else
            covariance(:,:,iblock,:) = nan(90,90,1,length(foi_range));
          end
          continue
        else
          error('Data corrupt?')
    ...
    save(sprintf([outdir 'pupmod_src_covariance_s%d_m%d_v%d.mat'],isubj,m,v),'covariance');
...
for ifoi = 1 : 21
[h,~,~,s]=ttest(Ct(:,:,:,2,2,ifoi),Ct(:,:,:,1,2,ifoi),'dim',3);
h_pos= h>0& s.tstat>0;
h_neg=h>0 & s.tstat<0;

pos(ifoi) = 100*sum(h_pos(mask))/sum(mask(:));
neg(ifoi) = 100*sum(h_neg(mask))/sum(mask(:));
