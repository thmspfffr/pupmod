...
% allpara.grid = 'coarse';
% foi_range = unique(round(2.^[1:.5:7]));
% para.segleng = ones(length(foi_range),1);
% --------------------------------------------------------
% VERSION 5
% --------------------------------------------------------
% v         = 5;
...
% timevariant     = 1;
% --------------------------------------------------------
% VERSION 11 - LPC with lpc jackknife
% --------------------------------------------------------
% v               = 11;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
...
% timevariant     = 1;
% --------------------------------------------------------
% VERSION 13 - orthopow with eloreta
% --------------------------------------------------------
v               = 14;
v_postproc      = 6;
v_grid          = 4; % 4 = aal
...
timevariant     = 0;
% --------------------------------------------------------
% VERSION 15 - orthopow with eloreta
% --------------------------------------------------------
% v               = 15;
% v_postproc      = 6;
% v_grid          = 4; % 4 = aal
...
%       
      if ~exist(sprintf([outdir 'pupmod_src_powcorr_ft_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
        system(['touch ' outdir sprintf('pupmod_src_powcorr_ft_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
      else
        continue
      end
...
%         dat = dat*filt;
%         
%         % GET AAL TIME COURSES
%         dat   = tp_grid2aal(dat,pars);
%         filt  = tp_grid2aal(filt,pars);
        ...
        pars.epleng    = size(dat,1);
        pars.aal       = 0;
        pars.epshift   = pars.epleng/2;
        
        % COMPUTE POWER CORRELATIONS
        % dat should be n x nchans
        if ~timevariant
          if ~lpc
            powcorr = tp_powcorr_ortho(dat,pars,filt,filt);
          else
...
%             powcorr = tp_data2lpc_jackknife(dat,pars,filt,filt);
       ...
       save(sprintf([outdir 'pupmod_src_powcorr_ft_s%d_m%d_b%d_f%d_v%d.mat'],isubj,m,iblock,ifoi,v),'powcorr');
